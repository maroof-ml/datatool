...
if "initialized" not in st.session_state:
    st.session_state.update({
        ...
        "max_auth_attempts": 3, "auth_mode": None, "registration_complete": False,
...
class DeepFaceTransformer(VideoTransformerBase):
    ...
    def transform(self, frame):
        ...
        try:
            result = DeepFace.extract_faces(rgb_frame, detector_backend="retinaface", enforce_detection=True)
...
def encode_face_from_image(image: Image.Image, model_name: str = "ArcFace") -> Tuple[Optional[np.ndarray], Optional[str]]:
    try:
        ...
        result = DeepFace.extract_faces(rgb_image, detector_backend="retinaface", enforce_detection=True)
...
def login_page():
    ...
    auth_type = st.radio("Login Method", ["Face Authentication", "Credentials"], horizontal=True)
    st.session_state.auth_mode = "face" if auth_type == "Face Authentication" else "credentials"
    ...
    with st.form("credentials_form"):
        ...
        if submit:
            if not username or not password:
                ...
                success, user_id = authenticate_credentials(username, password)
                if success:
                    ...
                    st.markdown(f'<div class="failed-auth">❌ Invalid credentials. {remaining} attempts remaining.</div>', unsafe_allow_html=True)
...
def face_login():
    ...
    if not registered_users:
        st.warning("⚠️ No users registered with face authentication. Please register first.")
        return
    st.info(f"👥 Registered Users: {', '.join(registered_users)}")
...
def df_to_latex_table(df: pd.DataFrame, max_rows=100) -> str:
    """
    Converting DataFrame to a LaTeX table with a maximum number of rows.
    """
    df = df.head(max_rows).copy()
    df = df.fillna('NaN')  # Replace NaN with string for LaTeX
...
def create_latex_document(df: pd.DataFrame, title: str = "DataPro Export") -> str:
    """
    Creating a complete LaTeX document for the DataFrame.
    """
...
if uploaded_file:
    ...
    with tab4:
        ...
        kind = st.selectbox("Visualization Type", ["Distribution", "Correlation", "Categorical", "Custom", "Pair Plot", "Time Series"])
    ...
    with tab6:
        ...
        if fmt == "CSV":
            ...
            data, mime, ext = latex_content, "text/latex", ".tex"  # Streamlit will handle LaTeX to PDF conversion
        st.download_button(
            f"📥 Download {fmt}",
            data=data,
